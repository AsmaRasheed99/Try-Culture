...
const PricingReact = () => {
  ...
  return (
      ...
      <div className="flex flex-col lg:flex-row gap-6 h-full sm:w-96 px-5">
        {pricingData.map((data, index) => (
          <div
            className={`flex flex-col h-full w-full py-6 px-5 sm:px-10   rounded-xl ${
              data?.isSelected
                ? `bg-[#07b1c6] text-black`
                : "bg-white text-black"
            }`}
            key={index}
          >
